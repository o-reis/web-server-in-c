...
#include <fcntl.h>
// Initializes all named semaphores used for process synchronization
int init_semaphores(semaphores_t *sems, int queue_size)
{
    ...
    sems->cache_mutex = sem_open("/ws_cache_mutex", O_CREAT, 0666, 1);
    if (sems->empty_slots == SEM_FAILED || sems->filled_slots == SEM_FAILED || sems->queue_mutex == SEM_FAILED || sems->stats_mutex == SEM_FAILED || sems->log_mutex == SEM_FAILED || sems->cache_mutex == SEM_FAILED)
    {
        return -1;
    }
    return 0;
}
// Closes and unlinks all named semaphores
